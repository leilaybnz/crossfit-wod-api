...
export const createNewMember = (newMember) => {
    const isAlreadyAdded = DB.members.findIndex((member) => member.name === newMember.name) > -1; //> /1 && if I try to add the same workout it will return a 201 without the newly inserted workout
...
export const updateMember = (memberId, changes) => {
    ...
    try {
        ...
        const updatedMember = {
            ...
            ...changes,
            updatedAt: new Date().toLocaleString('es-AR', {timeZone: 'America/Buenos_Aires'})
        };
